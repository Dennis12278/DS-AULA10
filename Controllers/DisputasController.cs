    ...
    public class DisputasController : ControllerBase
    {
        ...
        public async Task<IActionResult> AtaqueComArmaAsync(Disputa d)
        {
            try
            {
                ...
                dados.AppendFormat(" Pontos de Vida do atacnate : {0}. ", atacante.PontosVida);
                ...
                //Programação dps próximos pasos aqui
                return Ok(d);
            }
        ...
        public async Task<IActionResult> AtaqueComHabilidadesAsync(Disputa d)
        {
            try
            {
                ...
                else
                {
                    ...
                    dados.AppendFormat(" Pontos de vida o oponente: {0}. ", oponente.PontosVida);
        ...
        public async Task<IActionResult> Sorteio()
        {
            ...
            //Sorteio com nu,ero da quantidade de personagens
            int sorteio = new Random().Next(personagens.Count);

            //Busca n alista pelo indice sorteandp (Não é o ID)
            Personagem p = personagens[sorteio];

            string msg =
                string.Format("Nº Sorteado {0}. Perdonagem: {1}", sorteio, p.Nome);
        ...
        public async Task<IActionResult> DisputaEmGrupoAsync(Disputa d)
        {
            try
            {
                ...
                while (qtdPersonagensVivos > 1)
                {
                    ...
                    //Seleciona personagens com pontos de vida positivos, exceto o atacante esoclhido e depois faz sorteio.
                    ...
                    //declara e redefine a cada passgem do while o valor das variáveis que serão usadas.
                    int dano = 0;
                    string ataqueUsado = string.Empty;
                    string resultado = string.Empty;

                    //Sorteia entre 0 e 1: 0 é um ataque com arma e 1 é uma taque com habilidades
                    bool ataqueUsaArma = (new Random().Next(1) == 0);

                    if (ataqueUsaArma && atacante.Arma != null)
                    {
                        //Programção do ataque com arma caso o atacante possua arma (o != null) do if
                        ...
                        d.Narracao += resultado; //Cocantena o resultado com as narrações existentes.
                        d.Resultados.Add(resultado);//Adiciona o resultado atual na lsita de resultados.
                    }
                    else if (atacante.PersonagemHabilidades.Count != 0)//Verifica see o personagem tem habilidades
                    {
                        //Realiza os sorteio entre as habilidades existentes e na linha seguinte e seleciona.
                    ...
                    //Atenção: Aqui ficará a Programação da verificação de ataque usado e verificação se existe mais de um personagem vivo
                    ...
                    if (qtdPersonagensVivos == 1) //Havendo só um personagem vivo, existe um CAMPEÃO!
                    {
                        ...
                        d.Narracao += resultadoFinal; //Concatena o resultado
                        d.Resultados.Add(resultadoFinal);

                        break; //break vai parar o While.
                    }//Fim do While
                     //Código após o fechamento do While. Atualizar os pontos de vida,
